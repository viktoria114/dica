...
// GET promociones visibles con items
export const getListaPromocionesV = async (req: Request, res: Response): Promise<void> => {
  try {
    const query = `
      SELECT
        p.id            AS promo_id,
...
export const getListaPromocionesI = async (req: Request, res: Response): Promise<void> => {
  try {
    const query = `
      SELECT
        p.id            AS promo_id,
    ...
    res.status(500).json({ message: "Error al obtener promociones invisibles" });
  }
};
...
export const agregarItemPromocion = async (req: Request, res: Response) => {
 const client = await pool.connect();
  try {
    ...
    const promoExists = await client.query("SELECT id FROM promociones WHERE id = $1", [id]);
    if (promoExists.rows.length === 0) {
      await client.query("ROLLBACK");
      return res.status(404).json({ message: `No existe la promoción con id=${id}` });
    }

    // Verificar que exista el menú
    const menuExists = await client.query("SELECT id FROM menu WHERE id = $1", [id_menu]);
    if (menuExists.rows.length === 0) {
      await client.query("ROLLBACK");
      return res.status(404).json({ message: `No existe el menú con id=${id_menu}` });
    }
